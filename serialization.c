...
void serialization (struct test_str * input, void * output, int count)
{
    ...
    for(int i=0; i<BOX_SIZE; i++) {
        /*разыменовывание указателя input, указывающего на входные данные
        копирование приравниванием к элементу массива (какого массива?), который нигде
        не декларирован*/
        *(int *)pnt = input[i].c;
        /* это вообще что? а на что похоже? */
        pnt += sizeof(int);
...
int main( int argc, char* args[] )
{
    ...
    /* С чего счетчик стал указателем? каким указателем? */
