...
[COMMENT:gmm] - Неверно, начиная от "Лучше..". Проанализируй снова и поправь, возможно
тебе помогут закомментированные строки. Корректная версия верна как исправление первой
функции, но нет версии которая исправляет вторую. И следи за форматированием плиз.
...
#+BEGIN_SRC lisp
    ...
    (defun min_max (vec sortfun lst)
      (let ((indx 0))
        (let (( elt ))
          (setf elt (svref vec indx)) ;; why initialization outside of declaration (let)?
          (do ((i (- (length vec) 1) (- i 1))) ;; why counting comes from end?
              ((= i 0) (setf lst (cons elt lst)))
            (setf indx (+ 1 indx)) ;; why index is outside of do-declatation?
            (format t "~%i=~A" i)
  ...
  ;; What about empty vector?
  (min_max (vector ) #'> '())

  (min_max (vector 1) #'> '())
#+END_SRC

Что-то мне подсказывает что это не рекурсивная функция..
