...
#+BEGIN_SRC lisp
    ...
    (let* ((flat s)
      ...
      (if (> result 0)
          (let* ((ray-point (make-instance
                             ...
                             :y (+ (y pt) (* result yr))
                             :z (+ (z pt) (* result zr))))
                 (result (triangle-intersect flat ray-point)))
            (if result
                (progn
   ;;            (format t "ray-point x ~a y ~a z ~a" (x ray-point) (y ray-point)
  ;;                      (z ray-point))
                (values ray-point)))))))
  ...
  (defmethod normal ((s flat) (pt point))
      (values (Nx s) (Ny s) (Nz s)))

  (defmethod normal ((s triangle) (pt point))
    (let ((treangle s))
      (values (Nx treangle) (Ny treangle) (Nz treangle))))
