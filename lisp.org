  ...
  (defun render (pathname scn width height res)
    (let ((result (png:make-image height width 1)))
      ...
      (with-open-file (p pathname :direction :output :if-exists :supersede)
        ;; создаем заголовок файла
        (format p "P2 ~A ~A 255" (* res 100) (* res 100))
        (destructuring-bind (l m n) (array-dimensions result)
          (loop for i from (- l 1) downto 0 do
               (loop for j from (- m 1) downto 0 do
                    (loop for k from (- n 1) downto 0 do
                       ;; (format t "a[~a ~a ~a] = ~a~%" i j k (aref result i j k))
                         (print (aref result i j k) p))))))))

  (render "ccc.pgm" (init-scene) 100 100 1)
#+END_SRC
