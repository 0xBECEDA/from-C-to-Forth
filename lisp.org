...
=======
  ;; Класс точки в трехмероном пространстве
  (defclass point ()
    ((x :initarg :x :accessor x )
>>>>>>> 65e9c61f22f82c6670bf9c60cb21a7c7d4d6470e
     (y :initarg :y :accessor y)
     (z :initarg :z :accessor z)))

  ;; Класс луча в трехмероном пространстве
  ;; Состоит из точки (origin) единичного вектора направления (direction)
  (defclass ray ()
    ((origin     :initarg :origin     :accessor origin)
     (direction  :initarg :direction  :accessor direction)))

  ;; Класс поверхности, имеющей только цвет, от него наследуются
  ;; геометрические фигуры
  ;; sur-color принимает значения от 0 (черный) до 1 (белый)
  (defclass surface ()
    ((color :accessor sur-color
            :initarg :sur-color)))

  ;; Класс сферы, включающая в себя структуру surface
  ;; Чтобы определить сферу нам нужны ее X,Y,Z-координаты,
  ;; радиус и цвет (который определен в surface)
  (defclass sphere (surface)
    ((radius :initarg :sphere-radius  :accessor sphere-radius)
     (center :initarg :sphere-center  :accessor sphere-center)))
...
>>>>>>> 65e9c61f22f82c6670bf9c60cb21a7c7d4d6470e
  ...
  (defun defflat (x y z Nx Ny Nz d color)
...
<<<<<<< HEAD
  ...
  (defmethod intersect ((s flat) pt xr yr zr)
    (let ((flat (s flat)))
      (let ((num (+ ( * (Nx flat) (x pt)) ( * (Ny flat) (y pt)) ( * (Nz flat) (z pt))
                    (d flat))))
        (let ((den ( + (* (Nx flat) xr) (* (Ny flat) yr) (* (Nz flat) zr))))
          (let (( n ( - 0 (/ num den))))
          (if ( < n 0)
              (let (( p-intersect
                     (make-instance 'point :x (+ (x pt) (* n xr))
...
<<<<<<< HEAD
  ...
  (defun ray-test-flat (&optional (res 1))
    (setf *world* nil)
    (defflat 0 -150 -200 -100 -120 -130 -9 .8)
    (tracer (make-pathname :name "spheres.pgm") res))
...
#+BEGIN_SRC lisp
  ...
  (defclass point ()
    ((point-x :initarg :point-x :accessor point-x)
     ...
     (point-z :initarg :point-z :accessor point-z)))

  ;; Класс оболочки, имеющей только цвет, от него наследуются геометрические фигуры
  ;; color принимает значения от 0 (черный) до 1 (белый)
  (defclass surface ()
    ((surface-color :initarg :surface-color :accessor surface-color)))

  ;; Класс сферы, включающий в себя surface
  ;; Чтобы определить сферу нам нужны ее X,Y,Z-координаты,
  ;; радиус и цвет (который определен в surface)
  (defclass sphere (surface)
    ((sphere-radius :initarg :sphere-radius  :accessor sphere-radius)
     (sphere-center :initarg :sphere-center  :accessor sphere-center)))

  ;; Класс поверхности, определяемой через нормаль и точку
  (defclass plane (surface)
    ((plane-normal   :initarg :plane-normal    :accessor plane-normal)
     (plane-distance :initarg :plane-distance  :accessor plane-distance)))

  ;; Класс источника света
  (defclass light ()
    ((light-position :initarg :light-position :accessor light-position)))

  (defclass scene ()
    ((scene-eye      :initarg :scene-eye       :accessor scene-eye)
     (scene-surfaces :initarg :scene-surfaces  :accessor scene-surfaces)
     (scene-lights   :initarg :scene-lights    :accessor scene-lights)))

  (defclass ray ()
    ((ray-origin    :initarg :ray-origin     :accessor ray-origin)
     (ray-direction :initarg :ray-direction  :accessor ray-direction)))
  ...
  (defun dir-vector (p1 p2)
    (lm:normalise
     (lm:make-vector
      3
      :initial-elements (list (- (point-x p2) (point-x p1))
    ...
    (let ((color 0))
      (dolist (l (scene-lights scn))
        (let* ((vl (dir-vector int (light-position l)))
               (sr (make-instance 'ray :ray-origin int :ray-direction vl))
  ...
  (defmethod intersect ((s sphere) r)
    (let* ((c (sphere-center s))
           (n (minroot
               (+ (sq (lm:x (ray-direction r)))
                  (sq (lm:y (ray-direction r)))
                  (sq (lm:z (ray-direction r))))
      ...
      (if n
          (make-instance
           'point
           :point-x (+ (point-x (ray-origin r))
                       (* (- n 0.0000000001)
                          (lm:x (ray-direction r))))
           :point-y (+ (point-y (ray-origin r))
                       (* (- n 0.0000000001)
                          (lm:y (ray-direction r))))
           :point-z (+ (point-z (ray-origin r))
                       (* (- n 0.0000000001)
                          (lm:z (ray-direction r))))))))

  (defun point-to-vector (p)
    (lm:make-vector
     3
     :initial-elements (list (point-x p)
                             (point-y p)
                             (point-z p))))

  (defmethod intersect ((s plane) r)
    (let* ((vd (lm:dot-product (plane-normal s) (ray-direction r)))
           (v0 (- (+ (lm:dot-product
                      (plane-normal s)
                      (point-to-vector (ray-origin r)))
                     (plane-distance s))))
           (n (/ v0 vd)))
      (if (>= n 0)
          (make-instance
  ...
  (defmethod normal ((s plane) pt)
    (declare (ignore pt))
    (plane-normal s))

  (defun create-plane (xn yn zn dist color)
    (make-instance
     'plane
     :plane-normal (lm:normalise
                    (lm:make-vector 3 :initial-elements (list xn yn zn)))
     :plane-distance dist
     :surface-color color))
  ...
  (defun init-scene ()
    (let ((srfcs nil)
      ...
      (push (create-plane .6 `-.3 .001 400 .5) srfcs)
      (push (create-plane -.6  -1 .001 400 1) srfcs)
      ;;(push (create-plane 5 -1 .001 400 .5) srfcs)
      ;;(push (create-plane 3 0 .001 400 1) srfcs)
      ;; (push (create-sphere 0 -100 -1200 200 .8) srfcs)
      ...
      (do ((x -2 (1+ x)))
        ...
        (do ((z 2 (1+ z)))
            ((> z 7))
          (push (create-sphere (* x 200) 300 (* z -400) 80 .75) srfcs)))
      (push (make-instance
             'light
             :light-position (make-instance
                              'point
                              :point-x 0
                              :point-y -300
                              :point-z -950))
      ...
      (make-instance
       'scene
       :scene-eye (make-instance 'point :point-x 0 :point-y 0 :point-z 200)
       :scene-surfaces srfcs
       :scene-lights lghts)))
  ...
  (render "ccc222.png" (init-scene) 300 300 1)
#+END_SRC
