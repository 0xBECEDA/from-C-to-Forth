...
#+BEGIN_SRC lisp
  ...
  (defun dir-vector (p1 p2)
    (lm:normalise
     (lm:make-vector
      3
      :initial-elements (list (- (x p2) (x p1))
                              (- (y p2) (y p1))
                              (- (z p2) (z p1))))))

  (defun color-at (scn x y)
    (let* ((vr (dir-vector (scene-eye scn) (make-instance 'point :x x :y y :z 0)))
           (r (make-instance
               'ray
  ...
  (defun sphere-intersect (s r)
    (let* ((c (sphere-center s))
           (n (minroot (+ (sq (lm:x (ray-direction r))) (sq (lm:y (ray-direction r))) (sq (lm:z (ray-direction r))))
                       (* 2 (+ (* (- (x (ray-origin r)) (x c)) (lm:x (ray-direction r)))
                               (* (- (y (ray-origin r)) (y c)) (lm:y (ray-direction r)))
                               (* (- (z (ray-origin r)) (z c)) (lm:z (ray-direction r)))))
                       (+ (sq (- (x (ray-origin r)) (x c)))
                          (sq (- (y (ray-origin r)) (y c)))
                          (sq (- (z (ray-origin r)) (z c)))
                          (- (sq (sphere-radius s)))))))
      (if n
          (make-instance 'point :x (+ (x (ray-origin r)) (* (- n 0.0000000001) (lm:x (ray-direction r))))
                         :y (+ (y (ray-origin r)) (* (- n 0.0000000001) (lm:y (ray-direction r))))
                         :z (+ (z (ray-origin r)) (* (- n 0.0000000001) (lm:z (ray-direction r))))))))

  (defun point-to-vector (p)
    (lm:make-vector
     3
     :initial-elements (list (x p)
                             (y p)
                             (z p))))
  ...
  (defun init-scene ()
    (let ((srfcs nil)
      ...
      (push (make-instance
             'light
             :light-position (make-instance 'point :x 0 :y -350 :z -950))
            lghts)
      (make-instance
       'scene
       :scene-eye (make-instance 'point :x 0 :y 0 :z 200)
