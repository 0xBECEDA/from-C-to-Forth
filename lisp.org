...
#+BEGIN_SRC lisp
  ...
  (defmacro test (n &body body)
    `(defun rec_test (n &body body)
       (if (not (eql n 0))
           (progn
             ,@body
             (rec_test (- n 1) ,@body)))))
  ...
  ;; запуск (rec_test 3 (print n)) - в этом случае ошибка "переменная n не привязана"
  ;; (rec_test 3 (format t "Hi!")) - Hi! печатается только 1 раз, поскольку при рекурсивном
  ;; вызове body каждый раз дополнительно оборачивается в список. Эксприменты с апострофами и запятыми ничего не дал.
...
#+BEGIN_SRC lisp
  ...
  ;; маркрораскрытие (возвращает nil, format внутри цикла ничего
  ;; не печатает)
  ...
  ;; тестовый вариант макрораскрытия (здесь format тоже ничего не выводит)

  (let* ((n 3) (var 2) (expr ( * var 2)))
  (defun test (n &rest expr)
      (do ((i 0 (+ i 1))
           (+ 1 var))
          (( <= i n))
