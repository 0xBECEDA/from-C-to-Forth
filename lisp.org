...
+ Сравниваем длины списка "до" и "после" remove. (length, >)
+ Записываем результат в переменную. (setf)
+ Создаем точечную пару "элемент.количество повторений".
- объединяем в список (не получается)
+ Сортируем (sort) (пока что не написано)
  ...
  (defun compr (lst)
    (let ((lst3))
      ...
      (let ((len))
        ...
        (let ((new_lst))
          (setf new_lst (cons len (car lst3)))
          (format t " new list ~a ~& " new_lst)
          (let ((final_lst))
            (setf final_lst (append new_lst final_lst))
