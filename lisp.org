...
#+BEGIN_SRC lisp
  ...
  (defun generate-text (n &optional (prev '|.|))
    ...
    (if (zerop n)
        ...
        (terpri)
        ;; в противном случае получаем слово в next
...
произведена с помощью Henley (см. исходный код выше).
...
- сделать предложение по умолчанию
...
возможно, имеет смысл остаить хэш-таблицу. Тогда будет действительно
осуществляться генерация предложения, а не просто вставка слова в
неизменное предложение.

#+BEGIN_SRC lisp
  ;; сюда будет помещаться предложение
  (defparameter *words*)
  ...
  ;;чтение текста
  ;; на вход получаем путь к тексту и слово
  (defun read-text (pathname)
    ;; открываем файл на чтение
    (with-open-file (s pathname :direction :input)
      ;; создаем буфер исходя из идеи, что в обзаце 100 слов
  ...
  (defun see (symb)
    ...
    (let ((pair (cdr *words*)))
      ;; пара пустая?
      (if (null pair)
          ;; зачит это первое слово в предложении, так и запишем (слово. 1)
          (setf *words* (cons symb 1))
          ;; в противном случае добавляем следующее слово к списку слов
          (setf *words* (cons (cons symb (incf (cdr pair))) *words*)))))


#+END_SRC

Теперь организуем саму вставку слова
