...
Нужно вернуть теперь 2 элемента: самый "весомый" и первый "весомый
после него.
- проход по списку
- сранивниваем первый и второй элементы в списке
- если второй выиграл в сравнении, делаем его первым и сравниваем дальше с ним.
- если список кончился, то записываем в результаты элемент-победитель, и если
результат один, то делаем все то же самое, предварительно удалив элемент-победитель
из списка.
             ...
             (let* ((wins (car lst))
               ...
               (do ((i ( - (length lst) 1) (- i 1))
                 ...
                 (let* ((obj (nth indx lst))
                   ...
                   (if (> score max)
                      (if (null (nth ( + indx 1 ) lst))
                          (setf result (cons obj result))
                          (setf wins obj)))))
               (if (not ( > (length result) 1))
                   (progn
                     ( setf lst (remove wins lst :test #'equal))
                     (format t "lst  ~a result ~a " lst result)
                    (go top))
                   result))))))
...
#+BEGIN_SRC lisp
  ...
  (defun bin-search (obj vec &key start end key test)
      (if (and (not (zerop len)) (null start) (null end))
          (finder obj vec 0 (- (length vec) 1))
