...
Попытка написать макрораскрытие:

Test рекурсия.

Не понятно, почему на (null counter) реакции не было и счетчик уходил
в минус, а на (eql 0 counter) реакция есть и рекурсия работает
корректно.

В TEST2 body вычисляется только 1 раз, хотя функция срабатывает 2
раза.

Не понятно, как сделать корректное возвращаемое значение функции
вместо nil: код каждый раз будет разным, может понадобиться вернуть
конкретное значение, а не просто прекратить рекурсию.

Не понятна разница между комбинации обратным апострофом и запятой и
просто подстановкой параметров, как это происходит в функции обычно:
мы передаем параметр в функцию, а функция использует значение этого
параметра.

Не понятно, как корректно проверить, что подставляется в body, чтоб не
писать вывод пf dеременной B, которая встречается в коде из body, но в
самой функции она нигде не определена. Так что как только имя
переменной меняется, все падает, естетсвенно.
...
#+BEGIN_SRC lisp
  ...
  (defun test2 (counter &rest body)
    (format t "B before ~a ~&" B)
    (if (eql counter 0)
        nil
        (progn
          body
          (format t "B ~a counter ~a ~&" B counter )
          (test2 (- counter 1) body))))
  ...
  (let ((b 0))
    (test2 5 (test2 2 (setf B (+ B 1)))))
  ;; => Err:
  ;; The variable B is unbound.
  ;; [Condition of type UNBOUND-VARIABLE]

  ;; запуск (setf B 0) (test2 2 (setf B (+ B 1)))
...
#+BEGIN_SRC lisp
  ...
  (defmacro n_of (var num &rest expr)
    (let ((lst) (result))
      `(do ((i 0 (+ i 1))
            (+ var 1))
           ((<= i ,num) lst)
