...
#+BEGIN_SRC lisp
  (defstruct (node (:print-function
    ...
    elt
    (l nil :type (or nil integer))  ;; NB: type declarations
    (r nil :type (or nil integer)))

  (defun bst-init (size)
    (make-array size :fill-pointer 0 :element-type 'node))
...
=======
>>>>>>> 113a9ed0fe56dc229876596fdf35570209fe1e0e
  ...
  ;; ищет объект в дереве
  (print (bst-find 1 *mytree* #'<))

  ;; возвращает максимальный объект в дереве
  (bst-max *mytree*)

  ;; возвращает минимальный объект в дереве
  (bst-min *mytree*)

  ;; удаляет объект в дереве
  ;; принимает объект, дерево и функцию
  (bst-remove 9 *mytree* #'<)

  ;; превращает дерево в список
  ;; принимает ноду, дерево и список
  (bst-lst (aref *mytree* 0) *mytree* '())

  ;; чистка памяти
  ;; принимает fill-pointer, до которого надо надо очистить память
  ;; (двигаемся от конца к началу)
  (clean-memory 5 *mytree*)
