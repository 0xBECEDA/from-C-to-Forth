...
#+BEGIN_SRC lisp
  ...
  (defclass triangle (flat)
    ...
    (let ((s (make-instance 'sphere
      ...
      (push s *world*)
      s))
  ;; Чтобы построить плоскость, нужно указать координаты точки, нормаль к плоскости и
  ...
  (defun defflat (Nx Ny Nz d color)
    (let ((s (make-instance 'flat
                            ...
                            :sur-color color)))
      (push s *world*)
      ;; (format t "defflat normal Nx ~a Ny ~a Nz ~a ~&" (Nx s) (Ny s) (Nz s))
      s))
    ...
    (multiple-value-bind (x y z)
      ...
      (multiple-value-bind (x2 y2 z2)
        ...
        (let* ((vec-ab (make-instance 'point
          ...
          ;;   (format t "normal-triangle x ~a y ~a z ~a ~&"
          ;;   (x normal) (y normal) (z normal))
          (format t "AB x ~a y ~a z ~a | AC x ~a y ~a z ~a ~&"
                  (x vec-ab) (y vec-ab) (z vec-ab)
                  (x vec-ac) (y vec-ac)(z vec-ac))

          ;;(values (/ (x normal) 100) (/ (y normal) 100) (/ (z normal) 100))))))
          (values (x normal) (y normal) (z normal))))))

  ;; в одной из ссылок нашла, что чтобы узнать D, нужно воспользоваться формулой
  ;; Ax + By +Cz = -D, где ABC - нормаль, а XYZ - координаты любой из вершин треугольника
  ...
  ;; метод, работающий только со сферами. В будущем появится такое же метод для других
  ...
  ;;       (change-location (A triangle) (A triangle))
  ;;     (multiple-value-bind (xb yb)
  ;;         (change-location (A triangle) (B triangle))
  ;;       (multiple-value-bind (xc yc)
  ;;           (change-location (A triangle) (C triangle))
  ;;         (multiple-value-bind (xp yp)
  ;;             (change-location (A triangle) ray-point)
  ...
  ;;                  (result-a (check-point (x new-a) (y new-a) (x new-b) (y new-b)
  ;;                                         (x new-p) (y new-p)))
  ;;                  (result-b (check-point (x new-b) (y new-b) (x new-c) (y new-c)
  ;;                                         (x new-p) (y new-p)))
  ;;                  (result-c (check-point (x new-c) (y new-c) (x new-a) (y new-a)
  ;;                                         (x new-p) (y new-p))))
  ...
  ;;                  ;;(Px*By - Bx*Py) / (Cx*By - Bx*Cy)
  ;;                  (m (/ (- (* (x new-p) (y new-b)) (* (x new-b) (y new-p)))
  ;;                        (- (* (x new-c) (y new-b)) (* (x new-b) (y new-c))))))
  ;;             (let ((result 0))
  ;;               (if (and (> m 0) ( < m 1))
  ;;                   ;;l := (Px - m*Cx) / Bx
  ;;                   (let ((l (/ (- (x new-p) (* m (x new-c)))
  ;;                               (x new-b))))
  ;;                     (if (and (>= l 0) (<= (+ m l) 1))
  ;;                         (values (setf result 1))
  ;;                         (values result)))))))))))


  (defun check-point (vec_x  vec_y vec2_x vec2_y px py)
    ;;atx * (by - ay) + aty * (ax - bx) + ay * bx - ax * by
    (- (+ (* px (- vec2_y vec_y)) (* py (- vec_x vec2_x))
          (* vec_y vec2_x)) (* vec_x vec2_y)))
  ...
  (defun ray-test-flat (&optional (res 1))
    ...
    ;;(defflat 100 -150 -100 3000 .2)
    ;;(defflat 9 18 -2 300 1)
    (deftriangle a b c 3000 .8)
    ;;(defsphere 0 -300 -1200 200 .8)
     ;; (defsphere -80 -150 -1200 200 .7)
     ;;(defsphere 0 80 -1200 200 .9)
     ;; (do ((x -2 (1+ x)))
     ;;     ((> x 2))
     ;;   (do ((z 2 (1+ z)))
     ;;       ((> z 7))
